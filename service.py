...
async def post_user(db: Session, raw_data: schemas.PostUser):
    ...
    login_type:str = raw_data.login_type
    google_id:str = raw_data.google_id
    apple_id:str = raw_data.apple_id
    ...
    last_login:datetime.datetime = raw_data.last_login
    user_avatar:str = raw_data.user_avatar
    is_blocked:bool = raw_data.is_blocked
    password:str = raw_data.password


    record_to_be_added = {'id': id, 'name': name, 'email': email, 'phone': phone, 'gender': gender, 'streak': streak, 'user_id': user_id, 'apple_id': apple_id, 'password': password, 'google_id': google_id, 'created_at': created_at, 'is_blocked': is_blocked, 'last_login': last_login, 'login_type': login_type, 'updated_at': updated_at, 'user_avatar': user_avatar, 'country_code': country_code}
    new_user = models.User(**record_to_be_added)
    db.add(new_user)
    db.commit()
    db.refresh(new_user)
    user_inserted_record = new_user.to_dict()



    bs_jwt_payload = {
        'exp': int((datetime.datetime.utcnow() + datetime.timedelta(seconds=200000)).timestamp()),
        'data': user_inserted_record
    }

    jwt_secret_keyss_1 = jwt.encode(bs_jwt_payload, '"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWV9.TJVA95OrM7E2cBab30RMHrHDcEfxjoYZgeFONFh7HgQ".', algorithm='HS256')

    res = {
        'user_inserted_record': user_inserted_record,
        'jwt1': jwt_secret_keyss_1,
    }
    return res
...
async def post_post_comments(db: Session, raw_data: schemas.PostPostComments):
    id:int = raw_data.id
    comment_id:uuid.UUID = raw_data.comment_id
    post_id:uuid.UUID = raw_data.post_id
    user_id:uuid.UUID = raw_data.user_id
    ...
    record_to_be_added = {'id': id, 'comment': comment, 'post_id': post_id, 'user_id': user_id, 'comment_id': comment_id, 'created_at': created_at, 'updated_at': updated_at, 'status_alias': status_alias}
    new_post_comments = models.PostComments(**record_to_be_added)
    db.add(new_post_comments)
    db.commit()
    db.refresh(new_post_comments)
    post_comments_inserted_record = new_post_comments.to_dict()

    res = {
        'post_comments_inserted_record': post_comments_inserted_record,
    }
    return res
...
async def post_communities(db: Session, raw_data: schemas.PostCommunities , request: Request):
    ...
    res = {
        'communities_inserted_record': communities_inserted_record,
        'auth': auth,
